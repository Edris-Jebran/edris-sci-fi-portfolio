...
export default function NavBar() {
  const linkCls = "nav-link cursor-pointer hover:text-primary transition-colors"
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-black/40 backdrop-blur border-b border-white/10" role="banner">
      <nav className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between" aria-label="Primary">
        <div className="flex items-center gap-3">
          <img src={logo} className="h-6 w-auto pulse-glow" alt="logo" />
          <span className="font-display tracking-widest text-sm glow">EDRIS JEBRAN</span>
        </div>
